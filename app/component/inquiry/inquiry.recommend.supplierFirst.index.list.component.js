...
var InquiryRecommendSupplierFirstIndexListComponent = (function () {
    ...
    InquiryRecommendSupplierFirstIndexListComponent.prototype.downloadRecommendExcelReport = function (recommendIndex, event) {
        var downloadElement = $(event.target).parents("[anchorId='download_td']");
        downloadElement.each(function (index, element) {
        ...
        if (recommendIndex.indexId) {
            this.recommendService.downloadRecommendExcelReport(recommendIndex.indexId).subscribe(function (response) {
                var blob = new Blob([response.blob()], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;" });
                var name = recommendIndex.indexId + ".xlsx";
    ...
    InquiryRecommendSupplierFirstIndexListComponent.id = "inquiry_recommend_supplier_first_index_list_component";
    __decorate([
